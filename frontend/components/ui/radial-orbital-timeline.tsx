...
export default function RadialOrbitalTimeline({
  ...
  return (
    <div
      className={`w-full flex flex-col items-center justify-center bg-transparent overflow-hidden ${className}`}
      ref={containerRef}
    ...
    >
      <div className="relative w-full h-full flex items-center justify-center">
        ...
        >
          {/* Center orb */}
          <div className="absolute w-16 h-16 rounded-full bg-gradient-to-br from-[#EF3988] via-[#1083BD] to-[#DAF467] animate-pulse flex items-center justify-center z-10">
            ...
            return (
              ...
              >
                ...
                <div className={`
                  ...
                  ${isExpanded ? "bg-white text-[#FF6B35] border-white shadow-lg shadow-black/20 scale-150"
                    : isRelated ? "bg-white/60 text-[#FF6B35] border-white animate-pulse"
                ...
                {/* Expanded card */}
                {isExpanded && (
                  <Card className="absolute top-20 left-1/2 -translate-x-1/2 w-60 bg-[#1A2840]/95 backdrop-blur-lg border-white/20 shadow-xl shadow-black/30">
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-px h-3 bg-white/50" />
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-center">
                        <Badge className={`px-2 text-xs ${getStatusStyles(item.status)}`}>
                          {item.status === "completed" ? "АКТИВНО" : item.status === "in-progress" ? "СКОРО" : "В ПЛАН"}
                        </Badge>
                      </div>
                      ...
                      <div className="mt-4 pt-3 border-t border-white/10">
                        <div className="flex justify-between items-center text-xs mb-1">
                          <span className="flex items-center text-white/60">
                            <Zap size={10} className="mr-1" /> Популярност
                          </span>
                          <span className="font-mono text-white/60">{item.energy}%</span>
                        </div>
                        <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-[#EF3988] to-[#1083BD]"
                      ...
                      {item.relatedIds.length > 0 && (
                        <div className="mt-4 pt-3 border-t border-white/10">
                          <div className="flex items-center mb-2">
                            ...
                            <h4 className="text-xs uppercase tracking-wider font-medium text-white/70">Свързани</h4>
                          </div>
