...
router.post("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    log.push(`✓ ${clinicDefs.length} клиники`)
    ...
    log.push("✓ Отзиви добавени")

    log.push("✅ Seed завършен!")
    res.json({ ok: true, log })
  } catch (err: any) {
    res.status(500).json({ ok: false, error: err?.message ?? "Unknown error", log })
  }
})
