...
export function BookingModal({ isOpen, onClose, vetId, vetName, services, schedule }: BookingModalProps) {
  ...
  const confirm = async () => {
    ...
    const res = await fetch("/api/appointments", {
      ...
      body: JSON.stringify({ vetId, serviceId: service?.id, date: dt.toISOString(), petName, petSpecies, notes }),
    })
  ...
  const reset = () => { setStep(1); setService(null); setDate(null); setSlot(""); setPetName(""); setDone(false) }
                ...
                <div className="grid grid-cols-4 gap-2 max-h-64 overflow-y-auto pr-1">
                  {availDates.map((d, i) => (
                    <button key={i} onClick={() => { setDate(d); setSlot(""); setStep(3) }}
                      className={`p-2 rounded-xl text-center border transition-colors ${date?.toDateString() === d.toDateString() ? "bg-[#1083BD] text-white border-[#1083BD]" : "border-gray-200 hover:border-[#1083BD]/40"}`}>
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input value={petName} onChange={e => setPetName(e.target.value)} placeholder="Рекс, Мица..."
              ...
              <div className="flex gap-3 mt-5">
                <button onClick={() => setStep(3)} className="px-4 py-2.5 border border-gray-200 rounded-xl text-sm font-medium hover:bg-gray-50">
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <button onClick={confirm} disabled={!petName || saving}
                  className="flex-1 py-2.5 bg-[#1083BD] hover:bg-[#0d6fa0] text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-50 transition-colors">
